...
## v1.0.1 (2026-06-17)
- Добавлены методы `Client.GetOrdersStatus` и `Client.GetUpdatedAfter` для получения статусов заявлений

## v1.0.0 (2026-06-17)
- Обновление до спецификации API ЕПГУ v1.14
- Реализована потоковая загрузка в `Client.AttachmentDownload`
- Добавлен контекст в методы `apipgu.Client` и `aas.Client`
- Добавлена поддержка PKCS#7-подписи `client_secret` для методов ЕСИА
- Обновлен OAuth2-flow ЕСИА: добавлена проверка `state`
- Документированы требования к `client_certificate_hash`
- Актуализированы ссылки на руководящие документы
- Удален тестовый провайдер подписи `signature.Nop`
- Обновлены XML namespace для пакетов услуг СФР
