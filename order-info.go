...
type OrderStatus struct {
	...
	StatusId            int       `json:"statusId"`            // Код статуса. Используемая статусная модель ЕПГУ и ее соотнесение со статусной моделью конкретной услуги (ведомственными статусами) содержатся в спецификации соответствующей услуги.
	...
	StateOrgStatusCode  string    `json:"stateOrgStatusCode"`  // Код ведомственного статуса. Статусные модели конкретных услуг закреплены в соответствующих спецификациях. Ведомственными именуются те статусы, которые используются конкретными ведомственными ИС при изменении статуса заявления на ЕПГУ при взаимодействии через СМЭВ в соответствии с ВС определенной услуги.
