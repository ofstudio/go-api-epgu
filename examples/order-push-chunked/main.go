// Пример резервирования номера заявления и загрузки архива по частям.
//
// # Основные шаги процесса
//
//  1. Заполнение данных заявления
//  2. Резервирование номера заявления: POST /api/gusmev/order
...
func main() {
	...
	// === ШАГ 2 ===
	// Резервируем номер заявления: POST /api/gusmev/order
	orderId, err := apiClient.OrderCreate(context.Background(), accessToken, srv.Meta())
	if err != nil {
		log.Fatal(err)
	}
	log.Print("Успешно зарезервирован номер заявления на ЕПГУ: ", orderId)
